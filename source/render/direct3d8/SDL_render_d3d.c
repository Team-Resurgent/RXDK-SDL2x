...
D3D_CreateRenderer(SDL_Window* window, Uint32 flags)
{
	...
	/* Flags (widescreen/progressive) based on video settings */
	vidflags = XGetVideoFlags();

	if ((pparams.BackBufferWidth > 640 && ((float)pparams.BackBufferHeight / (float)pparams.BackBufferWidth != 0.75f)) ||
		((pparams.BackBufferWidth == 720) && (pparams.BackBufferHeight == 576))) {
		pparams.Flags |= D3DPRESENTFLAG_WIDESCREEN;
	}

	if (XGetVideoStandard() == XC_VIDEO_STANDARD_PAL_I) {
		pparams.FullScreen_RefreshRateInHz = (vidflags & XC_VIDEO_FLAGS_PAL_60Hz) ? 60 : 50;
	}
